...
namespace {
  ...
  bool combineCommonParents(std::vector<Ref<const PredictionContext>> &parents) {
...
Ref<const PredictionContext> PredictionContext::merge(Ref<const PredictionContext> a, Ref<const PredictionContext> b,
  ...
  if (aType == PredictionContextType::SINGLETON) {
    left = std::make_shared<ArrayPredictionContext>(std::static_pointer_cast<const SingletonPredictionContext>(std::move(a)));
  } else {
  ...
  if (bType == PredictionContextType::SINGLETON) {
    right = std::make_shared<ArrayPredictionContext>(std::static_pointer_cast<const SingletonPredictionContext>(std::move(b)));
  } else {
